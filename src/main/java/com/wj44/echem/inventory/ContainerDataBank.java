...
public class ContainerDataBank extends ContainerEChem
{
    ...
    public ContainerDataBank(InventoryPlayer playerInventory, TileEntityDataBank tileEntityDataBank)
    {
        ...
        this.addSlotToContainer(new Slot(tileEntityDataBank, tileEntityDataBank.DATA_CARD_INVENTORY_INDEX, 30, 30)
        {
            @Override
        ...
        for (j = 0; j < this.numRows; ++j)
        {
            for (k = 0; k < 9; ++k)
            {
                this.addSlotToContainer(new Slot(tileEntityDataBank, indexOffset + k + j * 9, 8 + k * 18, 18 + j * 18)
                {
                    @Override
                    public boolean isItemValid(ItemStack itemStack)
                    {
